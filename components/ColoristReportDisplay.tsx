...
export const ColoristReportDisplay: React.FC<ColoristReportDisplayProps> = ({ reportData, clientImage, onReset, imageFilterClass }) => {
    ...
    <div id="colorist-report-display" className="bg-gray-800 rounded-2xl p-6 h-full flex flex-col">
      <div className="flex-shrink-0 flex justify-between items-center pb-4 mb-4 border-b border-gray-700 gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onReset} className="px-4 py-2 bg-gray-700 text-white rounded-lg font-semibold hover:bg-red-600/80 transition-colors text-sm flex items-center gap-2">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center">
              ...
              <ZoomableImage src={clientImage} alt="Cliente - Antes" filterClassName={imageFilterClass} />
            </div>
            <div className="text-center">
              <h3 className="font-semibold mb-2">Resultado (IA)</h3>
              <ZoomableImage src={tryOnImage} alt="Cliente - Depois" filterClassName={imageFilterClass} />
            </div>
          </div>

          <div className="bg-gray-900/50 p-4 rounded-xl space-y-4">
            <h3 className="text-xl font-semibold text-emerald-300 border-b border-gray-700 pb-2 mb-3">Análise de Visagismo e Colorimetria</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InfoCard icon="color-palette" title="Subtom de Pele" value={report.visagismAndColorimetryAnalysis.skinTone} />
              <InfoCard icon="filter" title="Contraste Pessoal" value={report.visagismAndColorimetryAnalysis.contrast} />
            </div>
            <p className="text-gray-300 italic leading-relaxed pt-2">{report.visagismAndColorimetryAnalysis.recommendation}</p>
          </div>
