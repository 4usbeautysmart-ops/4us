...
export const CameraCapture: React.FC<CameraCaptureProps> = ({ onImageCapture }) => {
    ...
    if (isCameraOn) {
      ...
      const openCamera = async () => {
        try {
          ...
          stream = mediaStream; // Keep track for cleanup

          if (videoRef.current) {
            videoRef.current.srcObject = mediaStream;
            // Explicit play is a safeguard as autoPlay might be unreliable
          ...
          if ((err as Error).name === 'OverconstrainedError' || (err as Error).name === 'NotFoundError') {
             ...
             if (facingMode === 'environment') {
               setFacingMode('user'); // If back fails, try front
             } else {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center text-center gap-2 p-2 bg-gray-900/30 rounded-lg">
        <canvas ref={canvasRef} className="hidden"></canvas>
        
        <div className="w-full aspect-square bg-black rounded-lg overflow-hidden flex items-center justify-center my-2">
            {error ? (
              <p className="text-red-400 p-4 text-sm">{error}</p>
            ) : isCameraOn ? (
              <video 
                ref={videoRef} 
                autoPlay 
        ...
        <div className="flex items-center justify-center gap-4">
          {!isCameraOn ? (
            ...
            <>
              <button onClick={handleFlipCamera} className="p-2 bg-gray-700 text-white rounded-full font-medium hover:bg-gray-600 transition-colors flex items-center justify-center" title="Virar Câmera">
                <Icon name="refresh" className="w-5 h-5"/>
              </button>
              <button onClick={handleCapture} className="p-4 bg-emerald-600 text-white rounded-full font-medium hover:bg-emerald-500 transition-colors flex items-center justify-center text-lg shadow-lg" title="Capturar Foto">
                <Icon name="camera" className="w-6 h-6"/>
              </button>
              <button onClick={() => setIsCameraOn(false)} className="p-2 bg-gray-700 text-white rounded-full font-medium hover:bg-gray-600 transition-colors flex items-center justify-center" title="Desligar Câmera">
