...
// const INITIATE_URL = "https://4us-three.vercel.app/api/createSubscription";
...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const iniciarAssinatura = async (planType: "monthly" | "yearly") => {
    if (!userProfile || !userProfile.uid || !userProfile.email) {
      alert(
        "Os dados do seu perfil estão incompletos ou não puderam ser carregados. A assinatura não pode ser iniciada. Por favor, faça login novamente."
      );
      return;
    }
  ...
  const testimonials = [
    {
      quote:
        "Essa ferramenta revolucionou meu salão. O planejamento dos cortes ficou muito mais preciso e meus clientes amam ver a simulação do resultado!",
      name: "Juliana Alves",
      title: "Hairstylist em São Paulo, SP",
    },
    {
      quote:
        "Nunca mais errei um tom de 'morena iluminada'. A análise de colorimetria é incrivelmente precisa. Recomendo para todos os coloristas.",
      name: "Ricardo Mendes",
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 rounded-2xl p-8 w-full max-w-4xl flex flex-col shadow-2xl shadow-emerald-500/10 max-h-[90vh]"
      ...
      >
          ...
          {forceOpen && (
            <div className="text-center mb-6 bg-red-900/30 p-4 rounded-lg border border-red-500/50">
              ...
              <p className="text-red-200 mt-1">
                Por favor, escolha um plano para continuar usando o app.
              </p>
            </div>
          )}
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-gray-700/50 p-6 rounded-lg border-2 border-emerald-500 shadow-lg shadow-emerald-500/10 flex flex-col relative">
                <div className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full">
                  MAIS POPULAR
                </div>
                <h3 className="text-xl font-semibold text-white">
                  Plano Anual
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {testimonials.map((testimonial, index) => (
                <div key={index} className="bg-gray-900/50 p-6 rounded-lg">
                  ...
                  <p className="text-right mt-4 font-semibold text-emerald-300">
                    — {testimonial.name}
                  </p>
        ...
        <p className="flex-shrink-0 text-xs text-center text-gray-500 mt-8 pt-4 border-t border-gray-700">
          * Você será redirecionado para um ambiente de pagamento seguro. O
          checkout é real.
        </p>
      </div>
    </div>
