                        ...
                        <div key={savedPlan.id} className="bg-gray-700/50 rounded-lg p-4 flex flex-col justify-between">
                            <div>
                                <div className="grid grid-cols-2 gap-2 mb-3">
                                  <img src={`data:image/png;base64,${savedPlan.clientImage}`} alt="Cliente" className="w-full h-24 object-cover rounded-md" />
                                  <img src={`data:image/png;base64,${savedPlan.referenceImage}`} alt="Referência" className="w-full h-24 object-cover rounded-md" />
                                </div>
                                <h3 className="font-semibold text-gray-200 truncate">{savedPlan.report.cuttingPlan.styleName}</h3>
                                <p className="text-xs text-gray-400">Salvo em: {new Date(savedPlan.id).toLocaleDateString()}</p>
                            </div>
                            <div className="flex gap-2 mt-4">
                                <button onClick={() => onLoadPlan(savedPlan)} className="flex-1 px-3 py-2 bg-emerald-600 text-white rounded-md text-sm font-medium hover:bg-emerald-500 transition-colors">
                                    Carregar
                                </button>
