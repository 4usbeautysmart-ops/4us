...
export const ActionButtons: React.FC<ActionButtonsProps> = ({ 
  ...
  const [showAnimateInput, setShowAnimateInput] = useState(false);
  ...
  const handleAnimateSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (animationPrompt.trim()) {
      onAnimate(animationPrompt);
      setShowAnimateInput(false);
      setAnimationPrompt('');
    }
  };

  const toggleEdit = () => {
    const isOpening = !showEditInput;
    setShowEditInput(isOpening);
    if (isOpening) {
  ...
  const toggleFilters = () => {
    const isOpening = !showFilters;
    setShowFilters(isOpening);
    if (isOpening) {
      setShowEditInput(false);
      setShowAnimateInput(false);
    }
  ...
  const toggleAnimate = () => {
    const isOpening = !showAnimateInput;
    setShowAnimateInput(isOpening);
    if (isOpening) {
      setShowEditInput(false);
      setShowFilters(false);
    }
  ...
  return (
    <div id="action-buttons" className="bg-gray-800 rounded-2xl p-4">
      <div className="grid grid-cols-4 sm:grid-cols-4 lg:grid-cols-8 gap-4 text-gray-300">
        <ActionButton icon="video" text="Animar com Veo" onClick={toggleAnimate} disabled={!canAnimate} />
      ...
      {showAnimateInput && isReady && (
        <form onSubmit={handleAnimateSubmit} className="mt-4 flex gap-2 animate-fadeIn">
          <input
            type="text"
            value={animationPrompt}
            onChange={(e) => setAnimationPrompt(e.target.value)}
            placeholder="Ex: um sorriso sutil e cabelo ao vento"
      ...
      {showEditInput && isReady && (
        <form onSubmit={handleEditSubmit} className="mt-4 flex gap-2 animate-fadeIn">
          <input
            type="text"
            value={editPrompt}
